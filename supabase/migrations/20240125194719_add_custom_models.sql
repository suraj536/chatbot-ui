...
DROP POLICY IF EXISTS "Allow full access to own models" ON models;
CREATE POLICY "Allow full access to own models"
    ON models
    USING (user_id = auth.uid())
    WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Allow view access to non-private models" ON models;
...
DROP TRIGGER IF EXISTS update_model_workspaces_updated_at ON model_workspaces;
